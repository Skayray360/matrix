...
$embedCode = 1
try {
    $body = @{ model = "embeddinggemma:latest"; input = "validacion" } | ConvertTo-Json
    ...
    $embedCode = if ($dimension -eq 768) { 0 } else { 1 }
    Register-Step "embeddings" $embedCode "dimension=$dimension"
}
catch { Register-Step "embeddings" 1 "endpoint no disponible" }
...
if ($SkipE2E) {
    $results["e2e"] = [ordered]@{ status = "SKIPPED"; exit_code = 0; detail = "omitido por parametro" }
    Write-Warn "E2E omitido por parametro."
}
else {
...
foreach ($key in $results.Keys) {
    $color = if ($results[$key].status -eq "PASS") { "Green" } elseif ($results[$key].status -eq "SKIPPED") { "Yellow" } else { "Red" }
