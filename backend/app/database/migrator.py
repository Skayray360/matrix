...
def ensure_database_exists() -> tuple[str, bool]:
    ...
    try:
        ...
        with engine.connect() as conn:
            ya_existia = bool(
                conn.execute(text("SHOW DATABASES LIKE :n"), {"n": db_name}).first()
            )
            if not ya_existia:
                conn.execute(
                    text(
...
def inspect_ownership(engine) -> tuple[bool, set[str]]:  # noqa: ANN001
    ...
    Criterio: si existe ``schema_migrations`` con filas, la base es nuestra. Si
    esta vacia o no existe pero hay otras tablas, la base pertenece a otra
    aplicacion.
    """
    ...
    with engine.connect() as conn:
        registered = conn.execute(text("SELECT COUNT(*) FROM schema_migrations")).scalar_one()
    if registered:
        return True, set()
    return (not foreign), foreign
...
def database_ownership_problem() -> str | None:
    ...
    if es_nuestra:
        return None
    return "contiene tablas de otra aplicacion: " + ", ".join(sorted(foreign)[:6])
