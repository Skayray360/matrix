...
class EntraIdentityProvider(IdentityProvider):
    ...
    def _require_configured(self) -> None:
        missing = [
            ...
            for name, value in (
                ...
                ("ENTRA_REDIRECT_URI", self._settings.entra_redirect_uri),
            )
            if not value or value == "replace_me"
        ]
        if missing:
            raise ConfigurationError(
                "Entra ID no esta configurado. Faltan: " + ", ".join(missing)
            )
    ...
    def _exchange_code(self, code: str, code_verifier: str) -> dict[str, Any]:
        ...
        client_secret = self._settings.entra_client_secret.get_secret_value()  # noqa: S105
        ...
        try:
            ...
            raise UnauthorizedError(
                "No fue posible contactar al proveedor de identidad.", detail=str(exc)
            ) from exc
    ...
    def normalize_identity(self, claims: dict[str, Any]) -> NormalizedIdentity:
        ...
        allowed = self._settings.entra_allowed_group_list
    ...
    def status(self) -> IntegrationStatus:
        """Estado honesto: nunca ``CONNECTED_AND_VALIDATED`` sin conexion real."""
        settings = get_settings()
        if (
            settings.entra_tenant_id in PLACEHOLDER_VALUES
            or settings.entra_client_id in PLACEHOLDER_VALUES
            or settings.entra_redirect_uri in PLACEHOLDER_VALUES
        ):
