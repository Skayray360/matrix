...
def login(request: Request, db: Session = Depends(get_db)) -> RedirectResponse:
    ...
    if settings.auth_provider is not AuthProvider.ENTRA:
    ...
    return RedirectResponse(url=challenge.authorization_url, status_code=302)
...
def _upsert_user_from_identity(db: Session, identity: NormalizedIdentity) -> User:
    ...
    db.add(
        IdentityLink(
            ...
            tenant_id=get_settings().entra_tenant_id or None,
...
def _sync_entra_roles(db: Session, user: User, identity: NormalizedIdentity) -> None:
    ...
    managed_role_ids = set(
        db.execute(
            select(EntraGroupRoleMapping.role_id).where(
                EntraGroupRoleMapping.provider == "entra"
            )
        ).scalars().all()
    )
    ...
    if keys:
        mapped = db.execute(
            select(EntraGroupRoleMapping).where(
                EntraGroupRoleMapping.provider == "entra",
                EntraGroupRoleMapping.external_key.in_(keys),
            )
        ).scalars().all()
    ...
    existing = set(
        db.execute(select(UserRole.role_id).where(UserRole.user_id == user.id)).scalars().all()
    )
