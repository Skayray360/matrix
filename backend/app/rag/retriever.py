...
def deduplicate(candidates: list[ScoredPayload]) -> list[ScoredPayload]:
    ...
    for candidate in sorted(candidates, key=lambda c: c.score, reverse=True):
        ...
        if not text or text in seen_exact:
            continue
        if any(text in _normalize_for_dedup(str(k.payload.get("text", ""))) for k in kept):
            continue
        seen_exact.add(text)
    ...
    def retrieve(
        ...
        *,
        ...
        if include_private and conversation_id:
            private_filter = VectorStore.build_private_filter(
