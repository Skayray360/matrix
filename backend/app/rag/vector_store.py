    ...
    def search(
        ...
        *,
        ...
        try:
            ...
            raise QdrantUnavailableError(
                "Fallo la busqueda vectorial.", detail=str(exc)
            ) from exc
    ...
    def list_private_chunks(
        ...
        *,
        ...
        if limit < 1:
            return [], False
        self.ensure_collection(self._private)
        query_filter = self.build_private_filter(
        ...
        try:
            while len(collected) < target:
                ...
                collected.extend(
                    ScoredPayload(score=1.0, payload=dict(point.payload or {}))
                    for point in points
                )
            ...
            raise QdrantUnavailableError(
                "Fallo la recuperacion de adjuntos privados.", detail=str(exc)
            ) from exc
