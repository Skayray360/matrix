...
def test_instalador_acepta_metadata_oficial_de_uv_sin_relajar_version_o_target() -> None:
    ...
    assert re.fullmatch(pattern, "uv 0.11.33 (x86_64-pc-windows-msvc)")
    assert re.fullmatch(
        pattern,
        "uv 0.11.33 (fece32fc5 2026-07-28 x86_64-pc-windows-msvc)",
    )
    assert re.fullmatch(pattern, "uv 0.11.34 (x86_64-pc-windows-msvc)") is None
    assert re.fullmatch(pattern, "uv 0.11.33 (aarch64-pc-windows-msvc)") is None
    assert re.fullmatch(pattern, "uv 0.11.33\ncontenido inesperado") is None
