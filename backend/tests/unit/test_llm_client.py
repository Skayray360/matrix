...
from app.llm.ollama_client import MAX_TRANSIENT_RETRIES, OllamaClient
...
class TestReintentos:
    def test_un_5xx_se_reintenta_y_luego_funciona(self):
        intentos = {"n": 0}
        ...
        client = build_client(handler)
        assert client.chat(model="gemma4:latest", messages=[]).content == "ok"
        assert intentos["n"] == 2
    ...
    def test_los_reintentos_estan_acotados(self):
        ...
        assert intentos["n"] == MAX_TRANSIENT_RETRIES + 1
